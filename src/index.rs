...
trait GroestlcoinCoreRpcResultExt<T> {
  fn into_option(self) -> Result<Option<T>>;
}

impl<T> GroestlcoinCoreRpcResultExt<T> for Result<T, groestlcoincore_rpc::Error> {
  fn into_option(self) -> Result<Option<T>> {
    match self {
      Ok(ok) => Ok(Some(ok)),
...
impl Index {
  pub fn open(options: &Options) -> Result<Self> {
    let client = options.groestlcoin_rpc_client(None)?;
...
mod tests {
  ...
  #[test]
  fn missing_inputs_are_fetched_from_groestlcoin_core() {
    for args in [
    ...
    for context in Context::configurations() {
      ...
      assert_regex_match!(
        crate::subcommand::wallet::get_unspent_outputs(
          &crate::subcommand::wallet::groestlcoin_rpc_client_for_wallet_command(
            "ord".to_string(),
            &context.options,
          )
          .unwrap(),
