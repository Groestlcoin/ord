...
pub(crate) enum Subcommand {
  ...
  #[command(about = "List the first gros of each reward epoch")]
  Epochs,
  #[command(about = "Find a gro's current location")]
  Find(find::Find),
  #[command(subcommand, about = "Index commands")]
  Index(index::IndexSubcommand),
  #[command(about = "List the gros in an output")]
  List(list::List),
  #[command(about = "Parse a gro from ordinal notation")]
  Parse(parse::Parse),
  ...
  #[command(about = "Display Groestlcoin supply information")]
  Supply,
  #[command(about = "Generate teleburn addresses")]
  Teleburn(teleburn::Teleburn),
  #[command(about = "Display gro traits")]
