...
//! constructed transaction given the `Target`, which include the outgoing gro
  ...
  fn add_value(mut self) -> Result<Self> {
    ...
    if let Some(mut deficit) = total.checked_sub(self.outputs.last().unwrap().1) {
      while deficit > Amount::ZERO {
        ...
        if benefit > deficit {
          tprintln!("added {value} gro input to cover {deficit} sat deficit");
          deficit = Amount::ZERO;
        } else {
          tprintln!("added {value} gro input to reduce {deficit} sat deficit by {benefit} gro");
          deficit -= benefit;
        }
      }
    }
