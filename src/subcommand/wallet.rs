...
pub(crate) enum Subcommand {
  ...
  #[command(about = "List wallet gros")]
  Sats(sats::Sats),
  #[command(about = "Send gro or inscription")]
  Send(send::Send),
...
pub(crate) fn initialize(wallet: String, options: &Options, seed: [u8; 64]) -> Result {
  check_version(options.groestlcoin_rpc_client(None)?)?.create_wallet(
    &wallet,
    None,
    Some(true),
    None,
  ...
  let client = options.groestlcoin_rpc_client(Some(wallet))?;
...
pub(crate) fn groestlcoin_rpc_client_for_wallet_command(
  wallet_name: String,
  options: &Options,
) -> Result<Client> {
  let client = check_version(options.groestlcoin_rpc_client(Some(wallet_name.clone()))?)?;
