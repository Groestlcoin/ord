...
fn send_on_mainnnet_works_with_wallet_named_foo() {
  ...
  CommandBuilder::new(format!(
    "wallet --name foo send --fee-rate 1 grs1qw508d6qejxtdg4y5r3zarvary0c5xw7k3k4sj5 {txid}:0:0"
  ))
  .rpc_server(&rpc_server)
  .run_and_deserialize_output::<Output>();
}
...
fn send_addresses_must_be_valid_for_network() {
  ...
  .rpc_server(&rpc_server)
  .expected_stderr(
    "error: address tgrs1q6en7qjxgw4ev8xwx94pzdry6a6ky7wlfe9x8z6 belongs to network testnet which is different from required groestlcoin\n",
  )
  .expected_exit_code(1)
  .run_and_extract_stdout();
}
...
fn do_not_send_within_dust_limit_of_an_inscription() {
  ...
  CommandBuilder::new(format!(
    "wallet send --fee-rate 1 grs1qw508d6qejxtdg4y5r3zarvary0c5xw7k3k4sj5 {output}:329"
  ))
  .rpc_server(&rpc_server)
  .expected_exit_code(1)
...
fn send_btc_with_fee_rate() {
  ...
  CommandBuilder::new(
    "wallet send --fee-rate 13.3 grs1qw508d6qejxtdg4y5r3zarvary0c5xw7k3k4sj5 1grs",
  )
  .rpc_server(&rpc_server)
  .run_and_deserialize_output::<Output>();
...
fn wallet_send_with_fee_rate_and_target_postage() {
  ...
  CommandBuilder::new(format!(
    "wallet send grs1qw508d6qejxtdg4y5r3zarvary0c5xw7k3k4sj5 {inscription} --fee-rate 2.0 --postage 77000gro"
  ))
  .rpc_server(&rpc_server)
  .run_and_deserialize_output::<Output>();
